class Producto {
	...
	set stock(value){
		...
		if( Number.isInteger(value) ){
			...
			alert("Error,  stock invalido. Indicar solmente num. enteros")
		}
	}

	get precio(){
		return (this._precio * 1.21 ).tofixed8(2)
	}
	...
	mostrar(area){
		// window.document.write(`<p> Hay ${this._stock} unidades de
		// 						 ${this._nombre} que valen ARS ${this._precio} </p>`)

		let ficha = document.createElement("ul")

		let datos = `<li>Nombre: ${this._nombre}</li>
					 <li>Stock: ${this._stock}</li>
	...
	static Comparar(p1, p2){
		...
		if ( p1._precio < p2._precio )
			document.write(`El ${p2._nombre} es mas barato que el ${p1._nombre}`)
	}
}